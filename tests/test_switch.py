...
async def test_circuit_failed_command_reverts_optimistic_state(
    ...
    """Regression: a failed command must raise and drop optimistic state.

    The old background write swallowed every exception while the optimistic
    state could only be cleared by a push echo - which never arrives when the
    command failed - so the UI showed the wrong on/off state indefinitely.
    """
    from pyintellicenter import ICConnectionError
