...
- **Structural refreshes avoid duplicate heater scans** ([#165](https://github.com/joyfulhouse/intellicenter/pull/165); fixes #164) - The shared base entity now invalidates the add-time cache in `async_added_to_hass` before `super()` (applying to all entities), and climate and water-heater entities skip `LISTORD` nulling during structural refresh so a structural push performs one heater scan.
