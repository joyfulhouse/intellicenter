...
- **Service calls now wait for panel acknowledgement** ([#139](https://github.com/joyfulhouse/intellicenter/pull/139), [#150](https://github.com/joyfulhouse/intellicenter/pull/150); fixes #133, #143) - Switch, cover, climate, number, select, and dimmer-light handlers previously returned before the panel command completed, and the fire-and-forget path could only log a connection loss, timeout, or rejection after Home Assistant had reported success. These handlers now await the shared command path and raise `HomeAssistantError` when the panel does not acknowledge the change; failed optimistic state is rolled back. Automations that relied on fire-and-forget completion will now see those errors.
- **Unload and climate acknowledgement regressions are asserted directly (tests only)** ([#152](https://github.com/joyfulhouse/intellicenter/pull/152); fixes #140, #144) - The unload test only checked that `async_stop` was called, while the acknowledgement matrix omitted climate preset selection and the no-current-heater fallback. It now asserts that teardown is awaited and exercises both missing climate branches, preventing a dropped `await` from passing unnoticed.
- **Awaited climate control and unload ordering coverage completed (tests only)** ([#159](https://github.com/joyfulhouse/intellicenter/pull/159); fixes #153, #154) - The acknowledgement matrix still omitted climate's valid-current-heater branch, and an `assert_awaited_once` could not distinguish completed teardown from a spawned task. The matrix now covers that heater path, and the unload test requires an asynchronously yielding stop to finish before unload returns.

### Fixed

- **Failed platform unloads preserve the live coordinator** ([#137](https://github.com/joyfulhouse/intellicenter/pull/137); fixes #135) - `async_unload_entry` stopped the coordinator even when Home Assistant failed to unload a platform, leaving surviving entities attached to a torn-down connection and logging a misleading success. Coordinator teardown now runs only after all platforms unload successfully, so a failed unload can be retried without losing live updates.
- **Deferred entity builders survive ordinary updates before backfill** ([#138](https://github.com/joyfulhouse/intellicenter/pull/138); fixes #134) - A runtime-added object's single redispatch opportunity was consumed by any subsequent notification, so a `STATUS` change racing ahead of attribute backfill could permanently suppress pump telemetry and dependent controls. Redispatch bookkeeping now tracks newly arrived monitored keys and keeps retrying as keys arrive, while object removal prunes the pending state.
- **Falsy-first runtime telemetry creates its entity when usable** ([#149](https://github.com/joyfulhouse/intellicenter/pull/149); fixes #142) - Deferred-builder tracking treated a present but empty telemetry key as complete, so a runtime-added pump whose first power, speed, or flow value was empty never gained that sensor when the value became truthy. Empty tracked keys now remain pending until their first usable value and then trigger one redispatch.
- **Dimmer failures and optimistic echoes reconcile correctly** ([#150](https://github.com/joyfulhouse/intellicenter/pull/150); fixes #143, #145) - Brightness was the final fire-and-forget service path, a `LIMIT`-only dimmer echo could leave optimistic state stuck, and dependency-resolution fallback skipped water-heater memory side effects. Brightness now uses the awaited on/off command helper, dimmers treat `LIMIT` as an authoritative echo, fallback refreshes retain `isUpdated` side effects, and the obsolete fire-and-forget wrappers are removed.
- **Initial-connect telemetry gets the same deferred backfill as runtime additions** ([#157](https://github.com/joyfulhouse/intellicenter/pull/157); fixes #155) - Objects present at startup never entered deferred-builder bookkeeping, and slotted `OBJTYP`/`SUBTYP` fields made completeness checks impossible for tracked equipment such as pumps. Startup now seeds genuinely incomplete objects through the shared redispatch helper, excludes slotted fields from completeness, and prunes resolved entries.
- **Runtime redispatch bookkeeping stays bounded to incomplete objects** ([#160](https://github.com/joyfulhouse/intellicenter/pull/160); fixes #158) - Runtime discovery briefly parked even fully populated objects in the pending maps. It now applies the same incomplete-only filter as initial setup, while legitimately missing attributes remain pending only until the object is completed or removed.
- **Structural refreshes avoid duplicate heater scans** ([#165](https://github.com/joyfulhouse/intellicenter/pull/165); fixes #164) - Climate and water-heater entities now unify add-time cache invalidation in `async_added_to_hass` before `super()`, and skip `LISTORD` nulling during structural refresh so a structural push performs one heater scan. The behavior is symmetric across climate and water-heater entities.

### Performance

- **Push updates target only interested entities** ([#141](https://github.com/joyfulhouse/intellicenter/pull/141); fixes #136) - Every ordinary push previously scanned the full pool model and invoked every entity listener before each entity discarded unrelated changes. The coordinator now indexes object and cross-object dependencies, performs direct changed-object lookups, and routes ordinary updates only to interested listeners; structural and connection changes still rebuild the index and broadcast safely.
- **Repeated model scans are cached across structural epochs** ([#151](https://github.com/joyfulhouse/intellicenter/pull/151); fixes #146) - Entity-name reads and water-heater state access repeatedly traversed the model, while circuit subtype changes could leave friendly-name and light-show dependency caches stale. Same-kind counts and heater lists are now cached, structural invalidation refreshes them, and `SUBTYP` changes rebuild dependency edges before rendering.
- **Entity caches refresh at registration and avoid repeated climate heater scans** ([#163](https://github.com/joyfulhouse/intellicenter/pull/163); fixes #161, #162) - A name cache primed before coordinator-listener registration could miss the next structural invalidation, and climate preset reads rescanned all heaters on every access. Registration now clears pre-listener name counts, while climate heater lists are cached and invalidated on structural or ordering changes without changing preset behavior.
...
- **Dependent entities now retired when their equipment is removed** (#124) - Ghost-equipment removal (#113) only retired the entities *of* a deleted object; entities whose existence merely depended on it survived as permanent ghosts - a body's water heater after its last heater was deleted, a pump circuit's mode select and speed/flow numbers after the parent pump was deleted, and an IntelliChlor per-body output number after that body was deleted. The water_heater, select and number platforms now re-run their entity builders after a removal and retire tracked entities the builder no longer produces; the remaining platforms deliberately stay on own-object retirement because their builder predicates are not provably stable - sensor and friends gate on live telemetry values that can legitimately read empty, and climate's cooling-capability check accepts the heater `COOL` attribute, whose semantics are firmware-ambiguous between configuration and live cooling action (so a climate entity left behind by a deleted heat pump remains the one accepted ghost - reload the integration to retire it). Every platform additionally refreshes surviving entities' cross-object state, so a light group drops its color-effect capability when a member light is deleted. New-equipment dispatch now rebuilds over the full model, so a dependent entity (e.g. that IntelliChlor output number) is also recreated when its removed dependency returns. Known caveat: a heater deleted *and* replaced within a single reconnect retires and recreates the body's water heater (registry customizations are lost) because the replacement's body wiring only backfills after the removal dispatch - rare, and strictly better than a permanent ghost.
